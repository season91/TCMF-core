...
public static class Extensions
{
    ...
    #region 엔딩크레딧
      ...
      /// <summary>
    /// CanvasGroup(=Viewport + RectMask2D 역할)을 기준으로,
    /// 자식(Content)의 anchoredPosition.y를 올려서 크레딧 스크롤을 구현
    /// </summary>
    /// <param name="rootCanvasGroup">RectMask2D + CanvasGroup이 붙은 뷰포트 오브젝트</param>
    /// <param name="speedPxPerSec">px/sec 기준 속도 (SetSpeedBased=true)</param>
    /// <param name="topPadding">스크롤 종료 후 여유 패딩</param>
    /// <param name="fastKey">빠르게 감기 키</param>
    /// <param name="fastMul">빠르게 감기 배수</param>
    /// <param name="skipKey">스킵 키</param>
    /// <param name="onComplete">스크롤 완료 시 콜백</param>
    public static Tween ScrollCreditsFromRoot(
        this CanvasGroup rootCanvasGroup,
        float speedPxPerSec = 200f,
        float topPadding = 80f,
        KeyCode fastKey = KeyCode.LeftShift,
        float fastMul = 3f,
        KeyCode skipKey = KeyCode.Escape,
        Action onComplete = null)
    {
        // Viewport RectTransform (CanvasGroup이 붙은 루트)
        var viewport = rootCanvasGroup.GetComponent<RectTransform>();

        // 자식(Content) RectTransform — 실제 스크롤할 대상
        // 배경이 첫 자식일 수 있으므로, 레이아웃 컴포넌트가 붙은 자식을 탐색
        ...
        if (content == null)
        {
            Debug.LogWarning("[ScrollCreditsFromRoot] 콘텐츠를 찾지 못했습니다. (VerticalLayoutGroup/ContentSizeFitter 자식 없음)");
            return null;
        }

        // Layout 강제 갱신
        // VerticalLayoutGroup + ContentSizeFitter 조합에서 실제 높이를 정확하게 가져오려면 필요
        LayoutRebuilder.ForceRebuildLayoutImmediate(content);
        LayoutRebuilder.ForceRebuildLayoutImmediate(viewport);

        // 뷰포트(마스크 영역) 높이
        // float vpH = viewport.rect.height;
        // 콘텐츠 실제 높이 (ContentSizeFitter 적용 후 계산)
        float contH = Mathf.Max(content.rect.height, LayoutUtility.GetPreferredHeight(content));

        // 콘텐츠 시작 위치 = 화면 아래쪽(-뷰포트 높이)
        // var p = content.anchoredPosition;
        // p.y = -vpH;
        // content.anchoredPosition = p;

        // 도착 위치 = 콘텐츠 높이 + 패딩
        float endY = contH + topPadding;
        
        // Tween 생성: anchoredPosition.y를 위로 올림
        Tween tween = content.DOAnchorPosY(endY, speedPxPerSec)
                             .SetEase(Ease.Linear)
                             .SetSpeedBased(true);

        tween.OnUpdate(() =>
             {
                 tween.timeScale = Input.GetKey(fastKey) ? fastMul : 1f;
                 if (Input.GetKeyDown(skipKey)) tween.Kill(true);
             })
